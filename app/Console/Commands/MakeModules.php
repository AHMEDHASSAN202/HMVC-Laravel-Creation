...
class MakeModules extends Command
{
    ...
    {
        $service_provider_template = str_replace(['{{module_name}}', '{{module_name_small}}'], [$module_name, strtolower($module_name)], $this->get_stub('ServiceProvider'));
        $web_template = str_replace(['{{module_name}}', '{{module_name_small}}'], [$module_name, strtolower($module_name)], $this->get_stub('Web'));
        $api_template = str_replace(['{{module_name}}', '{{module_name_small}}'], [$module_name, strtolower($module_name)], $this->get_stub('Api'));
        ...
        file_put_contents($module_path . 'Resources' . DIRECTORY_SEPARATOR . 'Views' . DIRECTORY_SEPARATOR . strtolower($module_name) . DIRECTORY_SEPARATOR . 'test.blade.php', '<h1>'.$module_name.'</h1>');
    }

    protected function get_stub($type)
    {
